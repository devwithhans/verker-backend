...
module.exports = {
  ...
  async refreshJWT({}, req) {
    console.log(req.isUser, req.isVerker);
    if (!req.userId) {
      const error = new Error(errorName.NOT_VERKER);
      throw error;
    }
    ...
    console.log(company != null ? 'verker' : 'user');
  ...
  async createOutreach({
    ...
    for (const i in user.companyId.outreaches) {
      ...
      try {
        ...
        if (oldOffer) {
          await serverClient.updateMessage({
            id: offerInput.offerId, message: '', offer: oldOffer._id.toString(), user_id: offerInput.verkerId,
          });
        }
      } catch (error) {
        console.log(error);
